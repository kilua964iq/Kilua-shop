...
# Ø§Ù„Ø¨ÙˆØª Ø§Ù„Ø®Ø§Ø±Ù‚ - Ù…Ù…Ù„ÙƒØ© Ø±Ù‚Ù…ÙŠØ© Ù…ØªÙƒØ§Ù…Ù„Ø© Ù…Ø¹ Ø°ÙƒØ§Ø¡ Ø§ØµØ·Ù†Ø§Ø¹ÙŠ Ø¹Ø¨Ù‚Ø±ÙŠ
...
class TelegramHandler(logging.Handler):
    def emit(self, record):
        if record.levelno >= logging.ERROR:
            msg = f"â›” **Ø®Ø·Ø£ Ø¨Ø±Ù…ÙŠ:**\n`{record.getMessage()}`"
            # Send to admins with fix button
            loop = asyncio.get_event_loop()
            for admin_id in ADMIN_IDS:
                loop.create_task(bot.send_message(admin_id, msg, buttons=[[Button.inline("ðŸ”§ Ø¥ØµÙ„Ø§Ø­ ØªÙ„Ù‚Ø§Ø¦ÙŠ", b"fix_error_ai")]], parse_mode='md'))
