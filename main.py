# main.py - MUSTAFA SHOP v8 - الكود الكامل مع تعديل البروكسي فقط
# ============================================
...
# ============================================
# ✅ دالة فحص البروكسي المبسطة (بدون تعقيد)
# ============================================
...
async def check_proxy_handler(event, bot, supabase):
    try:
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            await event.respond(f"🌐 **IP السيرفر:** `{resp.text}`\n\n✅ الاتصال يعمل!", parse_mode='md')
    except Exception as e:
...
# دالة تدوير البروكسي (معطلة حالياً لأن البروكسي مش شغال)
async def auto_rotate_proxy():
    while True:
        try:
            await asyncio.sleep(60)
            # ملاحظة: هذه الدالة معطلة لأن البروكسي لا يعمل حالياً
...
async def main():
    ...
    # asyncio.create_task(auto_rotate_proxy())  # معطل حالياً
