...
# النسخة الكاملة 1700 سطر - بدون أي اختصار
...
# ============================================
# نظام تسجيل الأزرار المتقدم
# ============================================

class ButtonRegistry:
    """النظام الذكي لإدارة الأزرار - تحكم كامل من البوت"""
    ...
    async def execute(self, button_id: str, event, **kwargs):
        ...
        if button_id in self._static_handlers:
            try:
                ...
                log.error(f"خطأ في الزر الثابت {button_id}: {e}")
        ...
        if button_id in self._dynamic_buttons:
            ...
            try:
                ...
                log.error(f"خطأ في الزر الديناميكي {button_id}: {e}")
...
async def ai_create_button(event, bot, supabase):
    """ينشئ زراً جديداً بناءً على وصفك باللغة العربية"""
    if event.sender_id not in ADMIN_IDS:
...
async def budget_system(event, bot, supabase):
    """الواجهة الرئيسية لنظام الميزانية"""
    ...
    await event.respond(
        ...
        buttons=[
            ...
            [Button.inline("📊 سجل الإنفاق", b"spending_logs")],
...
# ============================================
# الأزرار الإضافية (اختصارات سريعة)
# ============================================
...
async def spending_logs(event, bot, supabase):
    ...
    for l in logs.data:
        card_num = l.get('payment_cards', {}).get('card_number', 'غير معروف') if l.get('payment_cards') else 'غير معروف'
...
# ============================================
# الأنظمة التكتيكية الجاهزة (15 نظام)
# ============================================

# نظام 1: الطيف (Ghost Mode)
...
# نظام 6: الخداع المعرفي (Cognitive Hack)
...
# نظام 8: القنبلة العنقودية (Cluster Bomb)
...
# نظام 12: التحكم العقلي (Mind Control)
...
# نظام 13: الخلايا النائمة (Sleepers)
