...
# النسخة النهائية الكاملة - مع حل مشكلة Proxy 407
...
# ============================================
# تفعيل البروكسي التلقائي لجميع المهام
# ============================================
...
# ============================================
# إعدادات البروكسي للـ Telethon نفسه
# ============================================
...
async def check_card_bin(card_number: str) -> dict:
    """فحص BIN للبطاقة - البروكسي مفعل تلقائياً"""
    try:
        bin = card_number[:6]
...
# ============================================
# نظام تسجيل الأزرار (المعدل لحل مشكلة البروكسي)
# ============================================
    ...
    async def execute(self, button_id: str, event, **kwargs):
        ...
        if button_id in self._dynamic_buttons:
            ...
            try:
                ...
                # ✅ إضافة البروكسي إلى المتغيرات المتاحة في الكود الديناميكي
                exec_globals = {
                    ...
                    'PROXY_URL': PROXY_URL,  # إضافة البروكسي
                    'USE_PROXY': PROXY_URL and USE_PROXY_FOR_ALL,
                    '__builtins__': __builtins__,
                }
                
                # ✅ إضافة سطر تفعيل البروكسي في بداية الكود المولد
                proxy_setup = f"""
# تفعيل البروكسي تلقائياً (لحل مشكلة 407)
import os
if '{PROXY_URL}' and os.environ.get('USE_PROXY_FOR_ALL', 'true') == 'true':
    os.environ['HTTP_PROXY'] = '{PROXY_URL}'
    os.environ['HTTPS_PROXY'] = '{PROXY_URL}'
    # إضافة التوثيق لـ httpx
    if '{PROXY_URL}' and '@' in '{PROXY_URL}':
        pass  # التوثيق موجود في الرابط
"""
...
async def start_handler(event):
    log.info(f"✅ تم استلام أمر /start من المستخدم {event.sender_id}")
...
async def setting_proxy(event, bot, supabase):
    await event.respond(
        ...
        "✅ جميع الأزرار والأوامر تستخدم البروكسي تلقائياً\n\n"
...
async def message_handler(event):
    ...
    if user_id in user_states and user_states[user_id].get("state") == "awaiting_button_data":
        ...
        if step == 1:
            ...
            state["step"] = 6
            state["data"] = data
            await event.respond("الخطوة 6/6: أرسل كود Python للزر (البروكسي سيعمل تلقائياً):")
        elif step == 6:
    ...
    if user_id in user_states and user_states[user_id].get("state") == "awaiting_ai_button_description":
        ...
        ملاحظة مهمة جداً: البروكسي سيعمل تلقائياً عبر متغيرات البيئة HTTP_PROXY و HTTPS_PROXY.
        لا تحتاج لإضافة proxy= في الكود، فقط استخدم httpx.AsyncClient() بشكل عادي.
