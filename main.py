...
# النسخة النهائية الكاملة مع تفعيل البروكسي التلقائي لجميع الأزرار
...
# ============================================
# ✅ تفعيل البروكسي التلقائي لجميع المهام والأزرار
# ============================================
...
# ============================================
# إعدادات البروكسي للـ Telethon نفسه (اختياري)
# ============================================

proxy_config = None
if PROXY_URL and not USE_PROXY_FOR_ALL:
    try:
        if "@" in PROXY_URL:
...
async def check_card_bin(card_number: str) -> dict:
    """فحص BIN للبطاقة - البروكسي مفعل تلقائياً عبر المتغيرات"""
    try:
        bin = card_number[:6]
...
async def message_handler(event):
    ...
    # تجاهل الرسائل من غير المطورين في وضع الإدارة
    ...
    if user_id in user_states and user_states[user_id].get("state") == "awaiting_button_data":
        ...
        if step == 1:
            ...
            # إضافة البروكسي إلى الكود تلقائياً إذا لم يكن موجوداً
            code_to_save = data["python_code"]
            if "httpx.AsyncClient" in code_to_save and "proxy=" not in code_to_save:
                # البروكسي سيعمل عبر متغيرات البيئة، لا حاجة لتعديل الكود
                pass
    ...
    if user_id in user_states and user_states[user_id].get("state") == "awaiting_ai_button_description":
        ...
        - ملاحظة: البروكسي سيعمل تلقائياً عبر متغيرات البيئة، لا تحتاج لإضافته في الكود
