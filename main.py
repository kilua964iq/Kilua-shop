# main.py - MUSTAFA SHOP v8 (Sovereign AI Engine) - الكود الكامل
# ============================================
...
async def check_proxy_handler(event, bot, supabase):
    try:
        ...
        await event.respond(f"❌ **خطأ في البروكسي:** `{str(e)[:100]}`", parse_mode='md')
...
async def callback_handler(event):
    try:
        ...
        # معالجة الاسترجاع من سلة المحذوفات
...
# ✅ أضف هذه الدالة الجديدة هنا (بعد auto_reset_accounts)
async def auto_rotate_proxy():
    """تغيير IP البوت تلقائياً كل دقيقة"""
    while True:
        try:
...
async def main():
    ...
    # ✅ أضف الـ task الجديد هنا مع المهام الأخرى
    ...
    asyncio.create_task(auto_rotate_proxy())  # <-- هذا سطر جديد، لا تحذف أي شيء
