...
async def check_proxy_handler(event, bot, supabase):
    try:
        ...
        # بيانات البروكسي من متغير البيئة
        proxy_url = PROXY_URL
        
        if not proxy_url:
            # فحص بسيط بدون بروكسي
        ...
        # استخراج بيانات البروكسي من الرابط (Taquito)
        # مثال: socks5://iEN2jEvl:5TqD95Nm664K@proxy.taquito.pp.ua:10080
        import re
        match = re.search(r'(socks5|http)://([^:]+):([^@]+)@([^:]+):(\d+)', proxy_url)
        
        if not match:
            await event.respond("❌ صيغة البروكسي غير صحيحة في متغيرات البيئة", parse_mode='md')
            return
        ...
        # عمل 3 اتصالات جديدة (كل اتصال يعطي IP جديد من Taquito)
        for i in range(3):
            try:
                ...
                await asyncio.sleep(1)  # انتظر ثانية بين الطلبات
                
            except Exception as e:
                await event.respond(f"⚠️ الطلب {i+1} فشل: `{str(e)[:50]}`", parse_mode='md')
        ...
        if len(set(ips)) > 1:
            ...
            await event.respond(
                ...
                f"قد تحتاج إلى:\n"
                f"• الانتظار 30-60 ثانية بين الطلبات\n"
                f"• أو إعادة تشغيل البوت",
                parse_mode='md'
            )
        ...
        await event.respond(
            "❌ **مكتبة مفقودة**\n\n"
            "قم بتثبيتها في Railway:\n"
            "```\npip install aiohttp-socks\n```\n"
            "أضف 'aiohttp-socks' إلى requirements.txt",
            parse_mode='md'
        )
    except Exception as e:
        await event.respond(f"❌ **خطأ:** `{str(e)[:200]}`", parse_mode='md')
