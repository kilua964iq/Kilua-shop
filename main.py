...
async def validate_python_code(code: str) -> tuple:
    """✅ جديد: فحص صحة كود Python قبل الرفع"""
    try:
...
async def get_system_context(user_id: int) -> str:
    ...
    return f"""=== سياق منظومة MUSTAFA SHOP ===
...
آخر الأخطاء: {error_text}
ملفات GitHub في {GITHUB_REPO}: {github_files_text}
...
# ============================================
# معالج الرسائل الموحد (إصلاح التكرار)
# ============================================
    ...
    if user_id in ADMIN_IDS and user_id in user_states:
        ...
        if current_state == "awaiting_button_data":
            ...
            if step == 1:
                ...
                state["step"] = 6
                await event.respond("✅ الخطوة 6/6: أرسل كود Python للزر (أو 'skip' لتركه فارغاً):")
            elif step == 6:
        ...
        if current_state == "awaiting_error_fix":
            ...
            fix = await ask_ai(f"الخطأ:\n{text}\n\nالكود الحالي (أول 1500 حرف):\n{(current_code or '')[:1500]}\n\nحلل الخطأ وأعطني الكود المصحح كاملاً داخل ```python```", user_id)
            await event.respond(
                f"🔧 **اقتراح الإصلاح:**\n\n{fix[:2000]}\n\nهل تريد تطبيقه؟",
        ...
        if current_state == "awaiting_ai_button_description":
            ...
            try:
                generated_code = await ask_ai(f"اكتب كود Python لزر تليجرام بهذا الوصف: {description}\nالكود يجب أن يستخدم: event, bot, supabase, Button, asyncio, datetime, random, json, httpx, log\nأخرج فقط الكود داخل ```python```", user_id)
        ...
        if data == "apply_fix":
            if user_id in user_states and user_states[user_id].get("state") == "awaiting_fix_apply":
                ...
                if code_match:
                    ...
                    if not is_valid:
                        await event.respond(f"⚠️ الكود فيه خطأ:\n`{error_msg}`")
                        del user_states[user_id]
                        return
...
async def individual_accounts_list(event, bot, supabase):
    ...
    if not accounts.data:
        await event.edit("📝 **لا توجد حسابات نشر فردي**", buttons=[[Button.inline("➕ إضافة", b"individual_add"), Button.inline("🔙 رجوع", b"admin_accounts")]])
        return
...
async def individual_add(event, bot, supabase):
    ...
    await event.respond(
        "➕ **إضافة حساب نشر فردي**\n\nاختر المنصة:",
        buttons=[
            [Button.inline("📱 تليجرام", b"ind_platform_telegram"), Button.inline("📘 فيسبوك", b"ind_platform_facebook")],
            [Button.inline("📷 إنستغرام", b"ind_platform_instagram"), Button.inline("🎵 تيك توك", b"ind_platform_tiktok")],
...
async def accounts_stats(event, bot, supabase):
    try:
        ...
        await event.edit(
            f"📊 **إحصائيات الحسابات**\n\n"
            f"💰 الترويج: {promo.count}\n"
            f"📝 النشر الفردي: {individual.count}",
...
async def setting_ai(event, bot, supabase):
    await event.respond(f"🤖 **إعدادات الذكاء الاصطناعي**\n\n🔑 مفتاح OpenAI: {'✅ موجود' if OPENAI_KEY else '❌ غير موجود'}\n💾 جدول الذاكرة: ai_memory", buttons=[[Button.inline("🔙 رجوع", b"admin_settings")]])

@registry.register("setting_proxy")
async def setting_proxy(event, bot, supabase):
    await event.respond(f"🌐 **إعدادات البروكسي**\n\n🔗 الحالة: {'✅ نشط' if PROXY_URL else '❌ غير مستخدم'}", buttons=[[Button.inline("🔙 رجوع", b"admin_settings")]])
        ...
        try:
            ...
            if status.get('status') == 'FAILED' and ADMIN_IDS:
                await bot.send_message(
                    ADMIN_IDS[0],
                    f"⚠️ **تنبيه Railway!**\n\nحالة النشر: فشل (FAILED)\nالوقت: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\nالسجلات: {status.get('logsUrl', 'غير متوفر')}"
                )
        except:
            await asyncio.sleep(3600)
