# main.py - MUSTAFA SHOP DIGITAL EMPIRE v5
# البوت العبقري - يتحدث معك بأي لغة، يحلل الأخطاء، ينفذ الأوامر، يدير الاستضافة
...
async def get_system_context(user_id: int) -> str:
    ...
    error_text = "\n".join([f"- {e.get('error_message', 'غير معروف')}" for e in errors.data]) if errors.data else "لا توجد أخطاء حديثة"
    
    # جلب الحالة من Railway
    ...
    حالة الاستضافة:
    - الحالة: {railway_status.get('status', 'غير معروف')}
    - آخر تحديث: {railway_status.get('createdAt', 'غير معروف')}
...
async def cmd_start(event, bot, supabase):
    ...
    await event.respond(
        ...
        "💡 **يمكنك التحدث مع المبرمج AI بأي لغة، فقط اكتب له ما تريد**",
        buttons=keyboard,
        parse_mode='md'
    )
...
async def main():
    log.info("🚀 جاري تشغيل MUSTAFA SHOP - DIGITAL EMPIRE v5...")
    log.info("=" * 50)
    ...
    if ADMIN_IDS:
        try:
            await bot.send_message(
                ADMIN_IDS[0],
                f"✅ **تم تشغيل MUSTAFA SHOP - DIGITAL EMPIRE v5**\n\n"
