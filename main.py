# main.py - MUSTAFA SHOP DIGITAL EMPIRE - الإصدار النهائي الذي يعمل
        ...
        async with httpx.AsyncClient(timeout=45) as client:
            resp = await client.post(
                ...
                json={
                    "model": "gpt-4o-mini",
                    "messages": [
                        {"role": "system", "content": "أنت خبير في Telethon. اكتب كود فقط."},
                        {"role": "user", "content": f"""
اكتب كود زر تليجرام بالشكل التالي فقط:

async def handler(event, bot, supabase, Button, asyncio, datetime, random, json, httpx, log):
    # المطلوب: {prompt}
    await event.respond("تم التنفيذ")

قواعد:
- لا تكتب import
- لا تكتب run_until_disconnected
- استخدم event.respond() للرد
- httpx.AsyncClient() للطلبات
"""}
                    ],
                    "temperature": 0.1
                }
...
"""
                ...
                exec_code = f"async def _dynamic_handler(event, bot, supabase, Button, asyncio, datetime, random, json, httpx, log, PROXY_URL):\n{indented_code}"
                
                exec(exec_code, exec_globals)
                await exec_globals['_dynamic_handler'](event, bot, supabase, Button, asyncio, datetime, random, json, httpx, log, PROXY_URL)
...
# ============================================
# ✅ الأمر السري لحذف أي زر (القوة المطلقة)
# ============================================

@registry.register("force_del")
async def force_delete_button(event, bot, supabase):
    if event.sender_id not in ADMIN_IDS:
        await event.answer("⛔ غير مصرح", alert=True)
        return
    parts = event.raw_text.split()
    if len(parts) != 2:
        await event.respond("❌ استخدم:\n/force_del button_id")
        return
    button_id = parts[1]
    # حذف الزر نهائياً
    supabase.table("buttons").delete().eq("button_id", button_id).execute()
    await registry.refresh_from_db(force=True)
    await event.respond(f"✅ **تم حذف الزر `{button_id}` نهائياً!**")

@bot.on(events.NewMessage(pattern='/force_del'))
async def force_del_cmd(event):
    await force_delete_button(event, bot, supabase)
...
async def message_handler(event):
    ...
    if user_id in user_states and user_states[user_id].get("state") == "awaiting_button_data":
        ...
        if step == 1:
            ...
            await admin_buttons(event, bot, supabase)
        return
    # معالج حذف زر من البوت مباشرة
if user_id in user_states and user_states[user_id].get("state") == "awaiting_delete_button_id":
    button_id = text.strip()
    supabase.table("buttons").delete().eq("button_id", button_id).execute()
    await registry.refresh_from_db(force=True)
    del user_states[user_id]
    await event.respond(f"✅ **تم حذف الزر `{button_id}` بنجاح!**")
    await admin_buttons(event, bot, supabase)
    return
    ...
    if user_id in user_states and user_states[user_id].get("state") == "awaiting_ai_button_description":
        ...
        try:
            ...
            folders_list = ["accounts", "tactical", "stealth", "ai_lab", "protection", "budget", "main"]
            keyboard = []
...
async def callback_error_handler(event):
    try:
        ...
        # اختيار مجلد للزر المُنشأ بالذكاء
        ...
        if data.startswith("edit_btn_"):
            ...
            if "edit_btn_code" in data:
                user_states[event.sender_id] = {"state": "awaiting_edit_code", "button_id": button_id}
                await event.respond(f"📝 **تعديل كود الزر**\n\nأرسل الكود الجديد للزر `{button_id}`:")
                return
            
            elif "edit_btn_name" in data:
                user_states[event.sender_id] = {"state": "awaiting_edit_name", "button_id": button_id}
                ...
                keyboard = []
                for folder in folders:
                    keyboard.append([Button.inline(f"{folder.get('emoji', '📁')} {folder.get('display_name', folder['folder_key'])}", f"move_btn_{button_id}_{folder['folder_key']}".encode())])
        ...
        if data.startswith("add_btn_in_"):
            ...
            await event.respond("➕ **إضافة زر جديد**\n\nالخطوة 1/6: أرسل الـ Button ID (معرف فريد):")
            return
