...
# ============================================
# ✅ دالة فحص البروكسي المتغيرة (النسخة المعدلة)
# ============================================
...
async def check_proxy_handler(event, bot, supabase):
    try:
        ...
        try:
            ...
            await event.respond(
                f"❌ **مكتبة aiohttp-socks غير مثبتة**\n\n"
                f"الخطأ: `{str(e)}`\n\n"
                f"قم بتثبيتها في Railway:\n"
                f"```\npip install aiohttp-socks\n```\n"
                f"أضف 'aiohttp-socks' إلى requirements.txt",
                parse_mode='md'
            )
            return
        ...
        if not proxy_url:
            async with aiohttp.ClientSession() as session:
                async with session.get("https://api.ipify.org", timeout=10) as resp:
                    ip = await resp.text()
                    await event.respond(f"🌐 **IP السيرفر:** `{ip}`\n\n⚠️ لا يوجد بروكسي مفعل", parse_mode='md')
            return
        
        # طباعة البروكسي للتشخيص (بدون كلمة المرور)
        proxy_preview = proxy_url.split('@')[-1] if '@' in proxy_url else proxy_url
        await event.respond(f"🔍 **جاري فحص البروكسي:** `{proxy_preview}`", parse_mode='md')
        
        # استخراج بيانات البروكسي
        import re
        match = re.search(r'(socks5|http)://([^:]+):([^@]+)@([^:]+):(\d+)', proxy_url)
        
        if not match:
            await event.respond(
                f"❌ **صيغة البروكسي غير صحيحة**\n\n"
                f"الرابط الحالي: `{proxy_preview}`\n\n"
                f"الصيغة المطلوبة:\n"
        ...
        await event.respond(f"✅ **تم التعرف على البروكسي:**\n• النوع: {proxy_type}\n• المضيف: {host}:{port}", parse_mode='md')
        ...
        for i in range(3):
            try:
                ...
                async with aiohttp.ClientSession(connector=connector) as session:
                    async with session.get("https://api.ipify.org", timeout=15) as resp:
                        ip = await resp.text()
                        ips.append(ip)
                        await event.respond(f"📡 **الطلب {i+1} - IP:** `{ip}`", parse_mode='md')
                ...
                await event.respond(f"⚠️ **الطلب {i+1} فشل:** `{error_msg[:100]}`", parse_mode='md')
        
        if len(set(ips)) > 1:
            await event.respond(
                f"✅ **البروكسي متغير بنجاح!**\n\n"
                f"ظهرت {len(set(ips))} عناوين IP مختلفة:\n"
                f"{chr(10).join([f'• {ip}' for ip in set(ips)])}",
                parse_mode='md'
            )
        elif ips:
            await event.respond(
                f"⚠️ **البروكسي لم يتغير**\n\n"
                f"جميع الطلبات أعطت نفس IP: `{ips[0]}`\n\n"
                f"قد تحتاج إلى إعادة تشغيل البوت.",
                parse_mode='md'
            )
        else:
            await event.respond(
                f"❌ **فشلت جميع محاولات الاتصال**\n\n"
                f"تأكد من:\n"
                f"1. صحة بيانات البروكسي\n"
                f"2. أن البروكسي لا يزال فعالاً\n"
                f"3. أن المنفذ ({port}) صحيح",
                parse_mode='md'
            )
            
    except Exception as e:
        error_msg = str(e) if str(e) else type(e).__name__
        await event.respond(f"❌ **خطأ عام:** `{error_msg[:200]}`", parse_mode='md')
...
# ✅ دالة تدوير البروكسي (تغير IP البوت كل دقيقة)
async def auto_rotate_proxy():
    while True:
        try:
            await asyncio.sleep(60)  # كل دقيقة
            log.info("🔄 تغيير IP تلقائي للبوت...")
            await bot.disconnect()
            await asyncio.sleep(2)
            await bot.connect()
            log.info("✅ تم تغيير IP البوت")
        except Exception as e:
...
async def main():
    ...
    asyncio.create_task(auto_rotate_proxy())  # ✅ تشغيل تدوير البروكسي
    ...
    log.info(f"🔄 تدوير البروكسي: {'🟢 مفعل (كل دقيقة)' if PROXY_URL else '⚪ غير مفعل'}")
    log.info("=" * 60)
