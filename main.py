# main.py - MUSTAFA SHOP DIGITAL EMPIRE v6 (FINAL FIXED)
...
# إعدادات GitHub - تم التصحيح
GITHUB_TOKEN = os.environ.get("GITHUB_TOKEN", "")
GITHUB_REPO = os.environ.get("GITHUB_REPO", "kilua964iq/Kilua-shop")  # ✅ تم التصحيح
...
# ============================================
# دوال GitHub والاستضافة المتقدمة (تم التصحيح)
# ============================================
...
async def list_github_files(path: str = "") -> str:
    """جلب قائمة الملفات من GitHub - ✅ تم التصحيح النهائي"""
    url = f"https://api.github.com/repos/{GITHUB_REPO}/contents/{path}?ref={GITHUB_BRANCH}"
    headers = {}
    if GITHUB_TOKEN:
        headers["Authorization"] = f"token {GITHUB_TOKEN}"
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                ...
                return f"❌ المستودع `{GITHUB_REPO}` غير موجود أو خاص. تأكد من:\n1. اسم المستودع صحيح\n2. المستودع عام (أضف GITHUB_TOKEN للمستودع الخاص)"
            else:
...
async def message_handler(event):
    ...
    if text.startswith('/'):
        if text == '/start' or text == '/start@' + (await bot.get_me()).username:
            ...
            await event.respond(f"📁 **المستودع الحالي:** `{GITHUB_REPO}`\n🌿 **الفرع:** `{GITHUB_BRANCH}`")
            return
        else:
            return
...
async def cmd_start(event, bot, supabase):
    ...
    await event.respond(
        ...
        "📁 **استخدم /github_repo لعرض اسم المستودع الحالي**",
        buttons=keyboard,
        parse_mode='md'
    )
...
async def ai_chat_button(event, bot, supabase):
    await event.respond(
        ...
        "• `/github_repo` - عرض اسم المستودع الحالي",
        parse_mode='md'
    )
...
async def admin_recycle(event, bot, supabase):
    ...
    if not deleted.data:
        ...
        return
    keyboard = []
    for item in deleted.data[:20]:
        keyboard.append([Button.inline(f"🔄 {item['item_type']}: {item['item_id'][:20]}", f"restore_{item['id']}")])
    keyboard.append([Button.inline("🔙 رجوع", b"admin_full_panel")])
...
# ============================================
# معالج الكالك باك الرئيسي (مختصر بسبب طول الكود)
# ============================================
...
# ============================================
# معالج الرسائل للمطور الذاتي (مختصر)
# ============================================
...
async def main():
    ...
    if GITHUB_TOKEN:
        log.info(f"🐙 GitHub متصل (مع توكن)")
    else:
        log.info("🐙 GitHub غير متصل (بدون توكن، سيتم الوصول للمستودعات العامة فقط)")
    ...
    log.info("📁 استخدم /github_repo لعرض اسم المستودع الحالي")
    ...
    if ADMIN_IDS:
        try:
            await bot.send_message(
                ...
                f"📁 **استخدم /github_repo لعرض اسم المستودع الحالي**"
            )
        except:
            pass
